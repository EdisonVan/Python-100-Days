...
3. 杜绝大事务、大 SQL、大批量、大字段等一系列性能杀手。
   ...
   - 大批量：多条 SQL 一次性执行完成，可减少一条条执行 SQL 产生的额外开销，但必须确保进行充分的测试，并且在业务低峰时段或非业务时段执行。
...
1. 主键列使用`unsigned`整数，可使用`auto_increment`，但是要禁止手动更新主键。
...
6. 单张表的字段数尽量空值在 50 个字段以内，如果字段过多可考虑垂直拆分。
...
10. 保存 IP 地址不要用`char(15)`，应该使用`int unsigned`，可使用`inet_aton`和`inet_ntoa`函数实现整数和 IP 地址的转换。
...
3. 如果为字符串类型创建索引，最好指定一个前缀长度，创建短索引。短索引可减少磁盘 I/O 而且在做比较时性能也更好，更重要的是 MySQL 底层的高速索引缓存能够缓存更多的键值。
...
过程，通常也称之为存储过程，它是事先编译好存储在数据库中的一组 SQL 的集合。调用存储过程可简化应用程序开发人员的工作，减少与数据库服务器之间的通信，对于提升数据操作的性能是有帮助的，这些在之前的[《关系型数据库 MySQL》](../Day36-40/36-38.关系型数据库MySQL.md)一文中已经提到过。

### 数据分区

MySQL 支持做数据分区，通过分区可存储更多的数据、优化查询，获得更大的吞吐量并快速删除过期的数据。关于这个知识点建议大家看看 MySQL 的[官方文档](https://dev.mysql.com/doc/refman/5.7/en/partitioning-overview.html)。数据分区有以下几种类型：
...
3. 通过`show profiles`和`show profile for query`分析 SQL。
   ...
   MySQL 从 5.0.37 开始支持剖面系统来帮助用户了解 SQL 执行性能的细节，可通过下面的方式来查看 MySQL 是否支持和开启了剖面系统。
   ...
   如果没有开启剖面系统，可通过下面的 SQL 来打开它。

   ```SQL
   set profiling=1;
   ```

   接下来就可通过剖面系统来了解 SQL 的执行性能，例如：
...
4. 优化 CRUD 操作。
   ...
   - 优化`insert`语句
     ...
     - 如果有多个连接向同一个表插入数据，使用`insert delayed`可获得更好的性能。
   ...
   - 优化`group by`语句
     ...
     - 在使用`group by`子句分组时，如果希望避免排序带来的开销，可用`order by null`禁用排序。

   - 优化嵌套查询

     - MySQL 从 4.1 开始支持嵌套查询（子查询），这使得可将一个查询的结果当做另一个查询的一部分来使用。在某些情况下，子查询可被更有效率的连接查询取代，因为在连接查询时 MySQL 不需要在内存中创建临时表来完成这个逻辑上需要多个步骤才能完成的查询。
   ...
   - 优化分页查询
     ...
     - 分页查询时，一个比较头疼的事情是如同`limit 1000, 20`，此时 MySQL 已经排序出前 1020 条记录但是仅仅返回第 1001 到 1020 条记录，前 1000 条实际都用不上，查询和排序的代价非常高。一种常见的优化思路是在索引上完成排序和分页的操作，然后根据返回的结果做表连接操作来得到最终的结果，这样可避免出现全表查询，也避免了外部排序。
...
1. 调整`max_connections`：MySQL 最大连接数量，默认 151。在 Linux 系统上，如果内存足够且不考虑用户等待响应时间这些问题，MySQL 理论上可支持到万级连接，但是通常情况下，这个值建议控制在 1000 以内。
...
4. 调整`innodb_lock_wait_timeout`：该参数可控制 InnoDB 事务等待行锁的时间，默认值是 50ms，对于反馈响应要求较高的应用，可将这个值调小避免事务长时间挂起；对于后台任务，可将这个值调大来避免发生大的回滚操作。
5. 调整`innodb_buffer_pool_size`：InnoDB 数据和索引的内存缓冲区大小，以字节为单位，这个值设置得越高，访问表数据需要进行的磁盘 I/O 操作就越少，如果可能甚至可将该值设置为物理内存大小的 80%。
