...
2. 修改`spiders`文件夹中名为`douban.py` 的文件，它是蜘蛛程序的核心，需要添加解析页面的代码。在这里，可通过对`Response`对象的解析，获取电影的信息，代码如下所示。
   ...
   通过上面的代码不难看出，可使用 CSS 选择器进行页面解析。当然，如果你愿意也可使用 XPath 或正则表达式进行页面解析，对应的方法分别是`xpath`和`re`。
...
3. 如果希望完成爬虫数据的持久化，可在数据管道中处理蜘蛛程序产生的`Item`对象。例如，可通过前面讲到的`openpyxl`操作 Excel 文件，将数据写入 Excel 文件中，代码如下所示。
