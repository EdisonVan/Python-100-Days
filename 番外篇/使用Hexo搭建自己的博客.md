...
对于一个程序员来说，搭建一个属于自己的博客平台是非常有意义的事情。首先，博客可记录自己的成长历程，也是对自己一段时间学习和工作的总结和沉淀；其他，通过博客可营销自己，增强自己在互联网或行业内的影响力，为将来更好的职业生涯打一个坚实的基础。前几年有一本名为《软技能 - 代码之外的生存指南》的畅销书，我记得书中有这么一段话：“流行乐队的音乐才华可能并不比夜店驻场乐队高多少，他们为什么就可在全世界巡回演出，创造一个又一个白金记录？……你的营销做得越好，你的才华才能表现得淋漓尽致。”

这里顺便啰嗦两句，在互联网如此发达的今天，应该如何营销自己呢？自我营销首先要从打造个人品牌做起，对于程序员来说，最容易去做好的一件事情还是搭建自己的博客。博客相当于是你在互联网上的一个基地，尤其是当你拥有了属于自己的独立博客后，你可做很多自己想做的事情，既可传达自己的思想，又可提升自己的影响力，当然如果你的博客经营得非常好，你可从中获利。当然，除了博客，直播、视频网站、投稿、写书、技术活动都是可选的自我营销方式。当然，做自我营销也需要持之以恒，三天打鱼两天晒网是很难有什么收获的。

### Hexo 概述

Hexo 是一个快速、简洁且高效的博客框架，它能够将[Markdown 格式](https://zh.wikipedia.org/zh-hans/Markdown)的文档渲染成漂亮的网页，这样就可在很短的时间内迅速的创建出网站的静态内容，而 Markdown 格式对程序员来说相信并不陌生。要想使用 Hexo 来搭建自己的博客，我实在想不出有什么教程比[官方文档](https://hexo.io/zh-cn/)更好，强烈建议大家阅读官方文档来了解 Hexo，下面我只做一个简要的使用说明。

> **Markdown**是一种轻量级标记语言，它允许人们使用易读易写的纯文本格式编写文档，同时也对图片、图表、数学公式提供了支持，可用来书写电子书、软件文档等，同时也可非常方便的转换为 HTML 页面或是 PDF 文档。

要使用 Hexo，首先得确保计算机上已经安装了[node.js](https://nodejs.org/en/)环境和[git](https://git-scm.com/)环境，前者是一个能够在服务器端运行 JavaScript 代码的环境，后者是版本控制工具。安装 node.js 主要是为了使用它的包管理工具 npm，所以不需要先系统的学习 node.js 的知识；而安装 git 是为了利用版本控制系统克隆代码已经将博客项目托管到三方平台，如果想学习 git，最好的资料是官方网站上的[_Git Pro_](https://git-scm.com/book/zh/v2)和[《Git 权威指南》](http://www.worldhello.net/gotgit/index.html)。安装完成后，可通过下面的命令来确认 node.js 环境以及它的包管理工具是否安装成功。
...
可通过下面的命令来检查是否安装了 git 环境。

```Shell
git --version
```

可使用 npm 来安装 Hexo，npm 是 node.js 的包管理工具，跟 Python 的 pip 工具作用一样，可用它来安装依赖库和三方工具。在第一次使用 npm 的时，可先将 npm 的下载源更换为国内的淘宝镜像，这样下载的速度会有非常显著的提升。
...
> Windows 环境命令行提示符中可使用 dir 命令查看目录结构。需要说明的是：`_config.yml`是博客项目的配置文件；`package.json`是项目的依赖项文件；`scaffolds`保存了 Markdown 文件的模板，也就是向新添加的 Markdown 文件中默认填充的内容；`source`目录下有一个名为`_post`的目录，稍后可将编写好的 Markdown 文件放到该目录，这样就可利用 Hexo 将 Markdown 文件处理成博客的静态页面，生成的静态页面将置于`public`目录下；`themes`文件夹保存了博客使用的主题。

通过下面的命令来安装项目所需的依赖项（`package.json`文件指明了这些依赖项）。

```Shell
npm install
```

做完上述的操作以后，已经可直接通过下面的命令来生成博客。
...
之前在安装依赖项的时包括了一个名为`hexo-server`的依赖项，该依赖项可帮助启动一个基于 node.js 的服务器来运行的博客项目，使用下面的命令即可启动服务器。
...
从运行命令的提示信息可看到，服务器已经运转起并使用了 4000 端口，可通过`Ctrl+C`来终止服务器的运行。如果要修改服务器使用的端口，可在启动服务器的时加上`-p`参数；如果希望服务器启动后，自动打开默认的浏览器访问服务器，可使用`-o`参数，如下所示。

```Shell
hexo s -p 8000 -o
```

至此，已经可看到 Hexo 在没有配置也没有加入自己的 Markdown 文件下生成的首页，如下图所示。
...
将编写好的 Markdown 文件可拷贝到`source/_posts`目录，可在每个 Markdown 文件的上方添加 Front-matter 来对文件的布局、标题、分类、标签、发布日期等信息加以说明。所谓 Front-matter，就是每个 Markdown 文件最上方以`---`分隔的区域，可在 Front-matter 中设置以下内容。
...
在完成上述工作后，可通过下面的命令先清理之前生成的内容。

```Shell
hexo clean
```

接下来就可用之前讲过的命令重新生成并运行博客项目。
...
虽然已经通过 GitHub 提供的域名访问到了自己的博客，但是如果不愿意“寄人篱下”，在利用 GitHub Pages 提供的托管服务同时，也可将博客绑定到自己专属的域名。如果暂时还没有购买域名，可在提供域名购买服务的网站（如：[万网](https://www.hichina.com/)、[GoDaddy](https://www.godaddy.com/)）上进行购买。

![](./res/wanwang.png)

> 目前国内对域名的管理日趋严格，在购买域名时需要填写一大堆的个人信息，进行实名认证后才能获得域名，这一点相信大家能够理解。

比如说，我现在已经购买了一个名为“jackfrued.top”的域名，如何让它跟“jackfrued.github.io”这个域名做一个绑定呢，可利用[阿里云控制台](https://dns.console.aliyun.com/)或[DNSPod](https://www.dnspod.cn/)来做一个域名解析服务。在域名解析平台登录成功后，可添加或选择自己的域名来配置域名解析。点击“添加记录”按钮，创建一个类型为 CNAME 的域名解析，CNAME 类型的解析代表将一个域名解析到另一个域名，如下图所示。
...
可清理一下之前生成的内容，将项目重新生成并发布到 GitHub 就大功告成了！现在已经拥有了独立域名的博客，希望大家可利用它做些有意义的事情（记录自己的成长历程、分享自己的工作经验、提升个人的影响力）。加油吧，程序员！
